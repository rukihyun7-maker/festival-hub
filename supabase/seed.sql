...
-- ============================================
-- 데모 계정 로그인 보강 (v3 추가)
-- 직접 시드한 auth.users가 최신 Supabase에서 이메일 로그인되도록:
--  1) auth.identities 레코드 생성  2) NULL 토큰 컬럼을 빈 문자열로
-- ============================================
...
select gen_random_uuid(), u.id, u.id::text,
  ...
  'email', now(), now(), now()
from auth.users u
where u.email like '%@festival.demo'
on conflict do nothing;
...
where email like '%@festival.demo';